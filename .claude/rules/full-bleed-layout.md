> Mirrors `.cursor/rules/full-bleed-layout.mdc` — keep both files in sync (see the `skills-sync` skill). Description: Only the header and footer run full width; every content section is capped and centerd

# Full-bleed layout

**Only the top navigation and the footer are ever full width.** Every content section — hero, skills, stacks, projects, toolkit, experience, education, awards, reading, recently-shipped — is capped and centerd, so the page reads as a stack of cards inset from the viewport edges.
